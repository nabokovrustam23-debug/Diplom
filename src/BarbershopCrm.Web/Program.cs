...
// Структурное логирование Serilog: запись в файл с ежедневной ротацией +
// обогащение запросным контекстом. До построения хоста — bootstrap-логгер,
// чтобы перехватывать ошибки старта.
...
try
{
    ...
    // Health-check для балансировщика/контейнера. Эндпоинт /healthz возвращает
    // 200 OK, если приложение и БД отвечают, и 503 при проблемах с БД.
    builder.Services.AddHealthChecks()
        .AddDbContextCheck<ApplicationDbContext>(name: "database");

    // Rate-limit: защищаем формы входа и публичной записи от перебора и спама.
    // Ключ — IP-адрес клиента (учёт за прокси через UseForwardedHeaders при необходимости).
    ...
    // Брендированные 404/500 страницы. Перехватываем коды ошибок и
    // переисполняем запрос на /Error/{code}.
    app.UseStatusCodePagesWithReExecute("/Error/{0}");

    app.UseHttpsRedirection();
    app.UseStaticFiles();

    // Защитные HTTP-заголовки на каждом ответе.
