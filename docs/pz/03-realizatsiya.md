...
На основе анализа результатов сравнения был сделан вывод, что в качестве целевой системы управления базами данных целесообразно выбрать Microsoft SQL Server 2019. При этом для локальной демонстрации и автоматизированной проверки в репозитории предусмотрен облегчённый режим запуска на SQLite: он не требует установленного SQL Server на машине проверяющего, но использует ту же модель Entity Framework Core и тот же прикладной код. В промышленном или учебно-лабораторном развёртывании провайдер переключается на SQL Server через параметры конфигурации `Database:Provider` и `ConnectionStrings:DefaultConnection`.
...
На основе ранее спроектированной ER-диаграммы (рисунок 2.1) для выбранной СУБД Microsoft SQL Server 2019 подготовлена база данных `BarbershopCrm`. Создание базы данных и её объектов выполняется воспроизводимо: классы предметной области, описанные в проекте `BarbershopCrm.Domain`, и их конфигурации в проекте `BarbershopCrm.Infrastructure` используются Entity Framework Core 8 для построения схемы, а справочный DDL-сценарий для SQL Server сохранён в файле `db/schema.sql`. Для демонстрационного запуска без установленного SQL Server приложение использует SQLite-файл `barbershop.db`, создаваемый автоматически при старте в среде разработки.
...
Фрагменты исходного кода ключевых классов приведены в Приложении Б, структура базы данных — в Приложении В, экранные формы пользовательского интерфейса — в Приложении Г, тест-кейсы автоматизированных тестов — в Приложении Д.
